...
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/taxonomia-ataque.pdf}
\end{center}
\end{frame}
...
ARP-scan
  ...
  \item Ejemplos:
    ...
    \item Escanear la red local de forma automática:\\
      {\scriptsize\cmd{sudo arp-scan -l}\quad o\quad \cmd{sudo arp-scan --localnet}}
    \item Escanear un rango de IPs concreto:\\
      {\scriptsize\cmd{sudo arp-scan --range=192.168.1.10-192.168.1.50}}
    \item Cambiar el tiempo de espera (timeout):\\
      {\scriptsize\cmd{sudo arp-scan --timeout=500 -l}}
...
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl022_1.jpg}
\end{center}
\small
...
Nmap
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.7\textwidth,height=0.5\textheight,keepaspectratio]{img/sl025_1.png}
\end{center}
\end{frame}
...
Tipos de escaneos en Nmap
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl026_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl023_1.png}
\end{center}
\end{frame}
...
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl024_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Lo primero que debemos hacer es detectar cuáles son los sistemas vivos en una determinada red. En caso de no encontrarnos en la misma red que los objetivos, podremos realizar los siguientes tipos de escaneo:
    ...
    \item ICMP Scan: utiliza paquetes ICMP para detectar si un equipo está vivo. En caso de existir medidas de seguridad o firewalls entre el auditor y el objetivo es posible que estos datos no sean verídicos.
      \begin{itemize}
      \item[] \cmdbox{nmap -sn [IP]}
      \end{itemize}
    \item Ping Sweep: básicamente es un barrido de ICMP scan en un determinado rango de direcciones IP.
      \begin{itemize}
      \item \cmd{nmap -sn [RANGO DE RED]} \quad (la opción \cmd{-sP} es el nombre antiguo de \cmd{-sn})
      \end{itemize}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\small
  ...
  \item La herramienta Nmap cuenta con multitud de tipos de escaneos diferentes:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.7\textwidth,height=0.45\textheight,keepaspectratio]{img/sl029_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl030_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl031_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl032_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item En caso de que existan medidas de seguridad, es posible utilizar otros escaneos alternativos, basados en la implementación de la pila TCP/IP. Los principales tipos son:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.5\textwidth,height=0.35\textheight,keepaspectratio]{img/sl035_1.jpg}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{Ejercicio}
\small
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl036_1.jpg}
\end{center}
  \begin{itemize}
...
\small
  ...
  \item Nmap permite realizar este tipo de fingerprinting de manera automática, y es capaz de detectar la versión de los servicios que hay corriendo en los puertos abiertos.
    \begin{itemize}
    \item Fingerprinting de servicios (\cmd{-sV}).
    \item Scripts para fingerprinting (\cmd{-sC}). Esto es realmente enumeración, y se verá más en profundidad en el tema 2.3.
    \end{itemize}
  \item Además del fingerprinting de servicios, es interesante también determinar el fingerprinting del sistema operativo y su versión.
    \begin{itemize}
    \item Fingerprinting de sistema operativo (\cmd{-O}).
    \end{itemize}
  \end{itemize}
\end{frame}
...
\small
  ...
  \item Existen otras muchas opciones útiles dentro de Nmap. A continuación se detallan algunas de ellas:
    \begin{itemize}
    \item Definir los puertos a escanear (\cmd{-p}):
      \begin{itemize}
      \item Todos los puertos: \cmd{-p-}
      \item Un rango: \cmd{-p 1-65535}
      \item Una lista concreta: \cmd{-p 80,443,8080,8443}
      \item Top principal de puertos: \cmd{--top-ports [número]}
      \end{itemize}
    \item Modificar el puerto de origen de los escaneos (\cmd{-g}): \cmd{-g 80}
    \item Modo verbose: \cmd{-v}
    \item Modo agresivo (\cmd{-A}): equivale a \cmd{-sV -O -sC --traceroute}
    \item Salida: \cmd{-oG} (grep), \cmd{-oN} (normal), \cmd{-oX} (XML), \cmd{-oA} (todos)
    \end{itemize}
  \end{itemize}
\end{frame}
...
Nmap: referencia rápida de comandos
  ...
  \item Descubrimiento de hosts:
    \begin{itemize}
    \item \cmd{nmap -sn 192.168.1.0/24} --- ping sweep (sin escaneo de puertos)
    \item \cmd{nmap -sn -PR 192.168.1.0/24} --- ARP scan en red local (más fiable)
    \end{itemize}
  \item Escaneo de puertos:
    \begin{itemize}
    \item \cmd{nmap -sS IP} --- SYN scan, rápido y semisilencioso (requiere root)
    \item \cmd{nmap -sT IP} --- TCP Connect, sin privilegios de root
    \item \cmd{nmap -sU -p 53,161,500 IP} --- UDP scan en puertos clave
    \item \cmd{nmap -p- IP} --- todos los puertos (1-65535)
    \item \cmd{nmap --top-ports 1000 IP} --- los 1000 puertos más comunes
    \end{itemize}
  \item Fingerprinting y scripts:
    \begin{itemize}
    \item \cmd{nmap -sV IP} --- versión de servicios
    \item \cmd{nmap -O IP} --- sistema operativo (requiere root)
    \item \cmd{nmap -A IP} --- agresivo: OS + versión + scripts + traceroute
    \item \cmd{nmap -sC IP} --- scripts por defecto
    \item \cmd{nmap --script vuln IP} --- buscar vulnerabilidades conocidas
    \end{itemize}
  \item Salida:
    \begin{itemize}
    \item \cmd{nmap -oA nombre IP} --- exportar en \cmd{.nmap}, \cmd{.gnmap} y \cmd{.xml} a la vez
...
\small
\begin{center}\includegraphics[width=0.6\textwidth,height=0.45\textheight,keepaspectratio]{img/sl042_1.jpg}
\end{center}
  \begin{itemize}
...
\small
  ...
  \item Netcat (conocido por su comando \cmd{nc}) es una herramienta de línea de comandos que sirve para leer y escribir datos a través de conexiones de red utilizando los protocolos TCP o UDP.
    \begin{itemize}
    \item Servidor: se crea un proceso en escucha en un puerto.
      \begin{itemize}
      \item[] \cmdbox{nc -lvp <puerto>}
      \end{itemize}
    \item Cliente: se usa netcat para comunicarse, en claro, con el servidor que está a la escucha:
      \begin{itemize}
      \item[] \cmdbox{nc <ip> <puerto>}
      \end{itemize}
    \end{itemize}
...
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl046_1.png}
\end{center}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl046_2.png}
\end{center}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl046_3.png}
\end{center}
\end{frame}
...
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl047_1.png}
\end{center}
\end{frame}
...
Netcat también nos puede servir para escanear puertos:
  ...
  \item Opciones:
    \begin{itemize}
    \item \cmd{-z} se utiliza para escanear.
    \item \cmd{-v} para añadir verbosidad.
    \item \cmd{-w 1} se utiliza para establecer el tiempo de espera de cada conexión en 1 segundo.
    \item \cmd{1-10000} define qué puertos escanear.
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.55\textwidth,height=0.34\textheight,keepaspectratio]{img/sl051_1.png}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{Ejercicio: TryHackMe}
\small
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl052_1.jpg}
\end{center}
  \begin{itemize}
...
\begin{frame}[allowframebreaks]{Ejercicio}
\small
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl053_1.jpg}
\end{center}
  \begin{itemize}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl056_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item En Kali los puedes encontrar con:
    \begin{itemize}
    \item[] \cmdbox{locate *.nse}
    \end{itemize}
  \item Ejecutar scripts individuales:
    \begin{itemize}
    \item[] \cmdbox{nmap --script [script.nse] [target]}
    \end{itemize}
  \item Ejecutar scripts por categorías:
    \begin{itemize}
    \item[] \cmdbox{nmap --script [categoria] [target]}
    \end{itemize}
  \item Ejecutar scripts de múltiples categorías:
    \begin{itemize}
    \item[] \cmdbox{nmap --script [categoria1,categoria2,...] [target]}
    \end{itemize}
  \item Solucionar problemas de scripts:
    \begin{itemize}
    \item[] \cmdbox{nmap --script [script] --script-trace [target]}
    \end{itemize}
  \item Actualizar la base de datos de scripts:
    \begin{itemize}
    \item[] \cmdbox{nmap --script-updatedb}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl058_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl059_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl060_1.png}
\end{center}
\end{frame}
...
\begin{frame}{Ejercicio}
\small
\begin{center}\includegraphics[width=0.6\textwidth,height=0.45\textheight,keepaspectratio]{img/sl061_1.jpg}
\end{center}
  \begin{itemize}
...
Distintos tipos de escaneo:
  ...
  \item Señales atípicas (\cmd{-sN}, \cmd{-sF}, \cmd{-sX}): envía paquetes con combinaciones raras de flags (NULL, FIN o XMAS) para atravesar ciertos firewalls sin levantar alertas básicas.
  \end{itemize}
\end{frame}
...
Fragmentación de paquetes y señuelos:
  ...
  \item La idea reside en trocear la cabecera del paquete a partir del campo MTU que introduce la pila TCP/IP. Por tanto, un escaneo que se podría hacer con unos pocos paquetes se realiza usando muchos más de tamaño más pequeño. Esto hace que el firewall intermedio no sea capaz de interpretar la cabecera completa, puesto que el paquete se reconstruye en el destino y no en el firewall.
    \begin{itemize}
    \item Fragmentación en Nmap: \cmd{-f} o \cmd{--mtu}
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.5\textwidth,height=0.32\textheight,keepaspectratio]{img/sl065_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Ejemplos de comandos Nmap para fragmentación y MTU:
    \begin{itemize}
    \item \cmd{nmap -f IP} --- fragmentos de 8 bytes (escapa del DPI básico)
    \item \cmd{nmap -ff IP} --- fragmentos de 16 bytes
    \item \cmd{nmap --mtu 8 IP} --- MTU personalizado (debe ser múltiplo de 8)
    \end{itemize}
  \item Combinado con otros tipos de escaneo:
    \begin{itemize}
    \item[] \cmdbox{nmap -sS -f --mtu 8 -p 80,443,8080 192.168.1.1}
    \item[] \cmdbox{nmap -sS -ff -T2 -p- 192.168.1.1}
    \end{itemize}
  \item Por qué funciona y cuáles son sus límites:
    ...
    \item Conviene combinarla siempre con \cmd{--data-length} o \cmd{-D} para mayor efectividad.
    \end{itemize}
  \end{itemize}
\end{frame}
...
Modificación de rutas:
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl067_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Modificación de direcciones IP o MAC:
    ...
    \item También se puede aplicar a la dirección física MAC.
      \begin{itemize}
      \item IP y MAC falsas en Nmap: \cmd{-S} y \cmd{--spoof-mac}
      \end{itemize}
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl068_1.png}
\end{center}
\end{frame}
...
Decoy Scan y Zombie Scan: anonimización del origen
  \begin{itemize}
  \item Decoy Scan (\cmd{-D}): envía el escaneo mezclado con direcciones IP falsas para ocultar el origen real del atacante.
  \item El IDS/firewall ve múltiples orígenes y no puede determinar cuál es el atacante real.
  \item Ejemplos:
    \begin{itemize}
    \item \cmd{nmap -D RND:10 IP} --- 10 IPs aleatorias como señuelo
    \item \cmd{nmap -D IP1,IP2,ME,IP3 IP} --- \cmd{ME} = posición de tu IP real entre los señuelos
    \item \cmd{nmap -D RND:5 -sS -f IP} --- combinado con SYN scan y fragmentación
    \end{itemize}
  \item Zombie (Idle) Scan (\cmd{-sI}): anonimato total. La IP real nunca aparece en el tráfico hacia el objetivo.
  \item Ejemplo:
    \begin{itemize}
    \item[] \cmdbox{nmap -sI zombie\_IP:puerto IP\_objetivo}
    \end{itemize}
  \end{itemize}
\end{frame}
...
Proxies
  ...
  \item Anonimización completa vía Proxychains + TOR. Ejemplo:
    \begin{itemize}
    \item \cmd{proxychains nmap -sT -Pn IP} --- solo TCP Connect funciona por TOR
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl070_1.png}
\end{center}
\end{frame}
...
Puerto de origen y timing:
  \begin{itemize}
  \item Puerto de origen: simular tráfico legítimo (\cmd{-g} / \cmd{--source-port}).
  \item Muchos firewalls permiten tráfico proveniente de puertos de servicios conocidos. Esto se puede simular con Nmap, por ejemplo:
    \begin{itemize}
    \item \cmd{nmap -g 53 IP} --- simula consulta DNS saliente (puerto 53)
    \item \cmd{nmap --source-port 80 IP} --- simula tráfico web HTTP
    \item \cmd{nmap -g 443 -sS IP} --- simula HTTPS, muy eficaz contra firewalls permisivos
    \end{itemize}
  \item Timing: evitar la detección por volumen o frecuencia. Ejemplos:
    \begin{itemize}
    \item \cmd{nmap -T0 IP} --- paranoid: 1 sonda cada 5 min (máximo sigilo)
    \item \cmd{nmap -T1 IP} --- sneaky: 1 sonda cada 15 s
    \item \cmd{nmap --scan-delay 2s IP} --- demora manual entre sondas
    \item \cmd{nmap --max-rate 10 IP} --- máximo 10 paquetes/segundo
    \end{itemize}
  \end{itemize}
\end{frame}
...
Datos adicionales
  ...
  \item Confundir la inspección profunda (DPI). Ejemplos:
    \begin{itemize}
    \item \cmd{nmap --data-length 25 IP} --- añade 25 bytes aleatorios al paquete
    \item \cmd{nmap --data-length 100 -sS IP} --- hace que los paquetes parezcan tráfico real
    \end{itemize}
  \end{itemize}
\end{frame}
...
\small
  ...
  \item Resumen de cómo lanzar estas opciones desde la herramienta Nmap:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl072_1.png}
\end{center}
\end{frame}
...
Tabla resumen de técnicas Nmap:
  \begin{itemize}
  \item Fragmentación \cmd{-f} / \cmd{--mtu 8} $\rightarrow$ evade: DPI e inspección de paquetes
  \item Decoy \cmd{-D RND:10} $\rightarrow$ evade: identificación del origen
  \item Zombie Scan \cmd{-sI zombie\_IP} $\rightarrow$ evade: todo (IP real invisible)
  \item Puerto origen \cmd{-g 53} / \cmd{--source-port} $\rightarrow$ evade: reglas por puerto fuente
  \item Timing \cmd{-T0} / \cmd{--scan-delay} $\rightarrow$ evade: detección por volumen/velocidad
  \item Datos adicionales \cmd{--data-length 25} $\rightarrow$ evade: firmas de paquetes en IDS
  \item MAC spoofing \cmd{--spoof-mac 0} $\rightarrow$ evade: filtros por dirección física
  \end{itemize}
\medskip
Comando combinado ``de bajo perfil'':
  \begin{itemize}
  \item {\scriptsize\cmd{nmap -sS -f --mtu 8 -T1 -D RND:5 --source-port 443 --data-length 20 -Pn -oA scan IP}}
  \end{itemize}
...
\small
  ...
  \item Usando los NSE de Nmap. Ejemplos:
    \begin{itemize}
    \item[] \cmdbox{nmap --script http-waf-detect IP}
    \item[] \cmdbox{nmap --script http-waf-fingerprint IP}
    \end{itemize}
  \item Cambio de User-Agent: por defecto, Nmap se identifica a sí mismo. Modifica esto usando \cmd{--script-args http.useragent="Mozilla/5.0..."} para simular tráfico de un navegador legítimo.
  \item Inspección de cabeceras HTTP (análisis pasivo):
    \begin{itemize}
    \item Cabecera \cmd{Server}: muchos WAF modifican esta cabecera revelando su identidad (por ejemplo, \cmd{Server: cloudflare} o \cmd{Server: BarracudaWAF}).
    \item Cabeceras personalizadas: algunos cortafuegos inyectan cabeceras propias en las respuestas HTTP, como \cmd{X-Sucuri-ID}, \cmd{X-CDN} o \cmd{X-Protected-By}.
    \end{itemize}
  \end{itemize}
\end{frame}
...
\small
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl074_1.jpg}
\end{center}
  \begin{itemize}
...
\scriptsize
  \begin{itemize}
  \item[] \cmdbox{sudo iptables -I INPUT -p tcp --tcp-flags ALL SYN -j REJECT --reject-with tcp-reset}
    \begin{itemize}
    \item Soluciones: \cmd{nmap -sF}, \cmd{nmap -sN}, \cmd{nmap -sX}
    \item Limpiar las reglas: \cmd{sudo iptables -F INPUT}
    \end{itemize}

  \item[] \cmdbox{sudo iptables -I INPUT -p tcp -m length --length 40 -j REJECT --reject-with tcp-reset}
    \begin{itemize}
    \item[] \cmdbox{sudo nmap -sF}
    \item \cmd{-sS} funciona: son 44 bytes (\cmd{--length 44})
    \item[] \cmdbox{sudo nmap --data-length 10}
    \end{itemize}

  \item Fragmentación de paquetes: \cmd{-f}
  \item[] \cmdbox{sudo iptables -I INPUT -p tcp -m length --length :256 -j REJECT --reject-with tcp-reset}
    \begin{itemize}
    \item[] \cmdbox{nmap --data-length 300 <tu\_objetivo>}
    \end{itemize}

  \item[] \cmdbox{sudo iptables -I INPUT -p tcp -m ttl --ttl-lt 64 -j REJECT --reject-with tcp-reset}
    \begin{itemize}
    \item[] \cmdbox{sudo nmap --ttl 65}
    \end{itemize}

  \item \cmd{sudo iptables -I INPUT -p tcp -j REJECT --reject-with tcp-reset} (rechaza todo el tráfico TCP)
  \item[] \cmdbox{sudo iptables -I INPUT -p tcp -j REJECT}
  \item[] \cmdbox{sudo iptables -I INPUT -p tcp --sport 1337 -j ACCEPT}
    \begin{itemize}
    \item[] \cmdbox{sudo nmap -g 1337}
    \end{itemize}

  \item Decoy: \cmd{sudo iptables -I INPUT -p tcp -j LOG --log-prefix "atacker log" --log-level=4}
    \begin{itemize}
    \item[] \cmdbox{nmap -sS -D ip,ME,ip -p 80 ipobjetivo}
    \item[] \cmdbox{tail -f /var/log/syslog -n 40}
    \end{itemize}

  \item Filtrar por MAC: \cmd{sudo iptables -A INPUT -m mac --mac-source 00:0F:EA:91:04:08 -j REJECT}
    \begin{itemize}
    \item[] \cmdbox{sudo macchanger -r eth1}
    \item[] \cmdbox{macchanger -s}
    \item O también: \cmd{sudo nmap --spoof-mac Apple} (o poner la MAC concreta; \cmd{0} $\rightarrow$ aleatoria)
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Ejercicio}
\small
\begin{center}\includegraphics[width=0.78\textwidth,height=0.6\textheight,keepaspectratio]{img/sl075_1.jpg}
\end{center}
  \begin{itemize}
